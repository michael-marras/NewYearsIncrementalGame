...
void Player::setX(float X) { this->posX = X; }

float Player::getX() { return this->posX; }

void Player::setY(float Y) { this->posY = Y; }

float Player::getY() { return this->posY; }
...
Direction Player::getPlayerDirection() { return this->playerDirection; }
...
PlayerAnimations
Player::RegisterPlayerAnimationsFromGrid(const char *sheetName, int cols,
                                         int rows, int startX, int startY,
                                         PlayerAnimations startId) {
  ...
  for (int row = 0; row < rows; row++) {
    for (int col = 0; col < cols; col++) {
      ...
      animationId =
          static_cast<PlayerAnimations>(static_cast<int>(animationId) + 1);
    }
  }
  return animationId;
}

void Player::RegisterPlayerAnimation(PlayerAnimations id, const char *sheetName,
...
Uint64 Player::getAnimationTime() { return this->animationTime; }

void Player::setAnimationTime(int animationTime) {
  this->animationTime = animationTime;
}

PlayerStates Player::getPlayerState() { return this->playerState; }
...
void Player::ClearInventory() { this->inventory.clear(); }
...
void Player::UnequipTool() { equippedToolId = -1; }
